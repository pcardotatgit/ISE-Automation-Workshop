...
def main():
    ...
    route="/main"
    env.level+="-"
    print("\n"+env.level,white("def main() in 5_pxgrid_get_anc_policies.py : >\n",bold=True))
    loguer(env.level+" def main() in 5_pxgrid_get_anc_policies.py : >")
    global username
    global password
    global host
    global port
    ...
    a=input('\nOkay Ready !, lets get the anc_policies')
    # Get the security groups
    r=requests.post(f"{rest_url}/getPolicies",
        verify=False,
    ...
    # save the response
    with open('./result/anc_policies.json','w') as file:
...
def parse_config_to_dict(config_json_file):
    ...
    '''
    env.level+='-'
    print(env.level,white('def parse_config_to_dict() in 5_pxgrid_get_anc_policies.py  : >\n',bold=True))
    loguer(env.level+' def parse_config_to_dict() in 5_pxgrid_get_anc_policies.py : > ')
...
def send_api_call_function(username,password,method,api_url,headers,payload,result_file_name):
    ...
    '''
    print()
    print(white('def send_api_call_function() in 5_pxgrid_get_anc_policies.py  : >\n',bold=True))
    # ===================================================================                                            
...
if __name__=="__main__":
    print(env.level,white("\nMAIN FUNCTION ( the get anc policies application starts here ): >",bold=True))
    with open("./debug/log.txt","w") as file:
        pass
    loguer(env.level+"APPLICATION STARTS - get anc policies")
    main()
    print(green('\nOK DONE : check result in [ ./result/anc_policies.json ]',bold=True))
