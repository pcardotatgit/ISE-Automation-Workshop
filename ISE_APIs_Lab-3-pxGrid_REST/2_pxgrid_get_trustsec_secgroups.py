...
#!/usr/bin/env python
'''
    description : Get tructsec security groups from ISE server
'''
import env as env
from crayons import *
...
def main():
    ...
    route="/main"
    env.level+="-"
    print("\n"+env.level,white("def main() in 2_pxgrid_get_trustsec_secgroups.py : >\n",bold=True))
    loguer(env.level+" def main() in 2_pxgrid_get_trustsec_secgroups.py : >")
    global username
    global password
    global host
    global port
    ...
    print(yellow('REFERENCE : https://github.com/cisco-pxgrid/pxgrid-rest-ws/wiki/TrustSec-configuration',bold=True),'\n')
    ...
    a=input('\nLets do a service lookup on the com.cisco.ise.config.trustsec service')
    ...
    a=input('\nOkay Ready !, lets get the trust security groups')
    ...
    # save the response
    with open('./result/trustsec_secgroups.json','w') as file:
...
def parse_config_to_dict(config_json_file):
    ...
    '''
    env.level+='-'
    print(env.level,white('def parse_config_to_dict() in 2_pxgrid_get_trustsec_secgroups.py  : >\n',bold=True))
    loguer(env.level+' def parse_config_to_dict() in 2_pxgrid_get_trustsec_secgroups.py : > ')
...
def send_api_call_function(username,password,method,api_url,headers,payload,result_file_name):
    ...
    '''
    print()
    print(white('def send_api_call_function() in 2_pxgrid_get_trustsec_secgroups.py  : >\n',bold=True))
    # ===================================================================                                            
...
if __name__=="__main__":
    print(env.level,white("\nMAIN FUNCTION ( the get trustsec secgroups application starts here ): >",bold=True))
    with open("./debug/log.txt","w") as file:
        pass
    loguer(env.level+"APPLICATION STARTS - get trustsec secgroups")
    main()
    print(green('\nOK DONE : check result in [ ./result/trustsec_secgroups.json ]',bold=True))
