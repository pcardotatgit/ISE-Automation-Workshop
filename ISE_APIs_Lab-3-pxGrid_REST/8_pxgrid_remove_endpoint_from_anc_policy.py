...
#!/usr/bin/env python
'''
    description : Reomve an Endpoint from a ANC Policy
'''
import env as env
from crayons import *
...
def main():
    ...
    route="/main"
    env.level+="-"
    print("\n"+env.level,white("def main() in 8_pxgrid_remove_endpoint_from_anc_policy.py : >\n",bold=True))
    loguer(env.level+" def main() in 8_pxgrid_remove_endpoint_from_anc_policy.py : >")
    global username
    global password
    global host
    global port
    ...
    print('\nOkay Ready !, lets REMOVE endpoint ',yellow("11:22:33:44:55:66",bold=True),' from ANC policy :',yellow("ANC-Quarantine",bold=True))
    a=input('\nPress Enter to continue')

    rest_url=f'https://{pxgrid_hostname}:8910/pxgrid/ise/config/anc/clearEndpointByMacAddress'
    ...
    # save the response
    with open('./result/remove_endpoint_from_anc_policy.json','w') as file:
...
def parse_config_to_dict(config_json_file):
    ...
    '''
    env.level+='-'
    print(env.level,white('def parse_config_to_dict() in 8_pxgrid_remove_endpoint_from_anc_policy.py  : >\n',bold=True))
    loguer(env.level+' def parse_config_to_dict() in 8_pxgrid_remove_endpoint_from_anc_policy.py : > ')
...
def send_api_call_function(username,password,method,api_url,headers,payload,result_file_name):
    ...
    '''
    print()
    print(white('def send_api_call_function() in 8_pxgrid_remove_endpoint_from_anc_policy.py  : >\n',bold=True))
    # ===================================================================                                            
...
if __name__=="__main__":
    print(env.level,white("\nMAIN FUNCTION ( the remove endpoint from anc policy  application starts here ): >",bold=True))
    with open("./debug/log.txt","w") as file:
        pass
    loguer(env.level+"APPLICATION STARTS - remove endpoint from anc policy ")
    main()
    print(green('\nOK DONE : Check Endpoint Assignments to ANC Policies in ISE and check result in [ ./result/remove_endpoint_from_anc_policy.json ]',bold=True))
