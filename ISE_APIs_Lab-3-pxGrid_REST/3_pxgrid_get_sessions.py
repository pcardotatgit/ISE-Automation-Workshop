...
#!/usr/bin/env python
'''
    description : Get currrent sessions from ISE server
'''
import env as env
from crayons import *
...
def main():
    ...
    route="/main"
    env.level+="-"
    print("\n"+env.level,white("def main() in 3_pxgrid_get_sessions.py : >\n",bold=True))
    loguer(env.level+" def main() in 3_pxgrid_get_sessions.py : >")
    global username
    global password
    global host
    global port
    # ===================================================================    
    print(yellow('REFERENCE : https://github.com/cisco-pxgrid/pxgrid-rest-ws/wiki/Session-Directory',bold=True),'\n')
    ...
    a=input('\nOkay Ready !, lets get the the sessions')
    ...
    # save the response
    with open('./result/sessions.json','w') as file:
...
def parse_config_to_dict(config_json_file):
    ...
    '''
    env.level+='-'
    print(env.level,white('def parse_config_to_dict() in 3_pxgrid_get_sessions.py  : >\n',bold=True))
    loguer(env.level+' def parse_config_to_dict() in 3_pxgrid_get_sessions.py : > ')
...
def send_api_call_function(username,password,method,api_url,headers,payload,result_file_name):
    ...
    '''
    print()
    print(white('def send_api_call_function() in 3_pxgrid_get_sessions.py  : >\n',bold=True))
    # ===================================================================                                            
...
if __name__=="__main__":
    print(env.level,white("\nMAIN FUNCTION ( the get sessions application starts here ): >",bold=True))
    with open("./debug/log.txt","w") as file:
        pass
    loguer(env.level+"APPLICATION STARTS - get sessions")
    main()
    print(green('\nOK DONE : check result in [ ./result/sessions.json ]',bold=True))
