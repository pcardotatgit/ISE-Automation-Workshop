...
#!/usr/bin/env python
'''
    description : Get All endpointf from the ISE server thanks to ISE Open APIs
'''
import env as env
from crayons import *
...
def main():
    ...
    route="/main"
    env.level+="-"
    print("\n"+env.level,white("def main() in 7_ise_ers_get_endpoint_open_apis.py : >\n",bold=True))
    loguer(env.level+" def main() in 7_ise_ers_get_endpoint_open_apis.py : >")
    global username
    global password
    global host
    global port
...
def parse_config_to_dict(config_json_file):
    ...
    '''
    env.level+='-'
    print(env.level,white('def parse_config_to_dict() in 7_ise_ers_get_endpoint_open_apis.py  : >\n',bold=True))
    loguer(env.level+' def parse_config_to_dict() in 7_ise_ers_get_endpoint_open_apis.py : > ')
...
def send_api_call_function(username,password,method,api_url,headers,payload,result_file_name):
    ...
    '''
    print()
    print(white('def send_api_call_function() in 7_ise_ers_get_endpoint_open_apis.py  : >\n',bold=True))
    # ===================================================================                                            
...
if __name__=="__main__":
    print(env.level,white("\nMAIN FUNCTION ( the get all_endpoints open apis application starts here ): >",bold=True))
    with open("./debug/log.txt","w") as file:
        pass
    loguer(env.level+"APPLICATION STARTS - get all_endpoints open apis ")
    main()
    print(green('\nOK DONE : check result in [ ./result/get_all_endpoints_open_apis.json ]',bold=True))
