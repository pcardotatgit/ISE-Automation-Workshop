...
result_file_name='create_anc_policy_result.json'
relative_url='/ers/config/ancpolicy' # <<<<<<<<<<<<<<<<<<<<<<<<<<< Relative URL here
...
if __name__ == "__main__":
    ...
    payload_dict={"ErsAncPolicy":{"name": "DISCONNECTED","actions": ["SHUT_DOWN"]}} 
    payload="{\"ErsAncPolicy\": {\"name\": \"DISCONNECTED\", \"actions\": [\"SHUT_DOWN\"]}}"
    #payload=json.dumps(payload_dict)
    #payload = "{\r\n    \"ErsAncPolicy\": {\r\n    \"name\": \"DISCONNECTED\",\r\n    \"actions\":[\"SHUT_DOWN\"]}}"
    print('payload : \n',cyan(payload_dict,bold=True))
  
    api_url=f"https://{host}:{port}{relative_url}"

    result,response = send_api_call_function(username,password,method,api_url,headers,payload_dict,result_file_name)
