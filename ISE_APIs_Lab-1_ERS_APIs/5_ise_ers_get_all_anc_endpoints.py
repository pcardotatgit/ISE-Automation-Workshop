...
#!/usr/bin/env python
'''
    description : Get All ENdpoint in All ANC Policies from the ISE server
'''
import env as env
from crayons import *
...
def main():
    ...
    route="/main"
    env.level+="-"
    print("\n"+env.level,white("def main() in 5_ise_ers_get_all_anc_endpoints.py : >\n",bold=True))
    loguer(env.level+" def main() in 5_ise_ers_get_all_anc_endpoints.py : >")
    global username
    global password
    global host
    global port
    ...
    result_file_name='get_all_anc_endpoints.json'
    method='get'
    relative_url='/ers/config/ancendpoint?size=100' 
...
def parse_config_to_dict(config_json_file):
    ...
    '''
    env.level+='-'
    print(env.level,white('def parse_config_to_dict() in 5_ise_ers_get_all_anc_endpoints.py  : >\n',bold=True))
    loguer(env.level+' def parse_config_to_dict() in 5_ise_ers_get_all_anc_endpoints.py : > ')
...
def send_api_call_function(username,password,method,api_url,headers,payload,result_file_name):
    ...
    '''
    print()
    print(white('def send_api_call_function() in 5_ise_ers_get_all_anc_endpoints.py  : >\n',bold=True))
    # ===================================================================                                            
...
if __name__=="__main__":
    ...
    main()
    print(green('\nOK DONE : check result in [ ./result/get_all_anc_endpoints.json ]',bold=True))
